RSpec.describe DbCharmer::Sharding::Method::DbBlockMap do
  ...
  describe "standard interface" do
    ...
    it "should return a shard config to be used for a key" do
      expect(@sharder.shard_for_key(1)).to be_kind_of(Hash)
    end

    it "should have shard_connections method and return a list of db connections" do
      expect(@sharder.shard_connections).to_not be_empty
    end
  end
  ...
  it "should correctly return shards for all blocks defined in the mapping table" do
    ...
    blocks.each do |blk|
      ...
      expect(shard[:connection_name]).to match(/social.*#{blk['shard_id']}$/)
  ...
  describe "for non-existing blocks" do
    ...
    it "should create a new one" do
      expect(@sharder.shard_for_key(@max_id + 1)).to_not be_nil
    end

    it "should assign it to the least loaded shard" do
      expect(@sharder.shard_for_key(@max_id + 1)[:connection_name]).to match(/shard.*03$/)
    end

    it "should not consider non-open shards" do
      @conn.execute("UPDATE event_shards_info SET open = 0 WHERE id = 3")
      expect(@sharder.shard_for_key(@max_id + 1)[:connection_name]).to_not match(/shard.*03$/)
    end
    ...
    it "should handle duplicate key errors" do
      ...
      expect(@sharder).to receive(:block_for_key).twice.and_return(nil, actual_block)
  ...
  it "should fail on invalid shard references" do
     ...
     expect(lambda { @sharder.shard_for_key(1) }).to raise_error(ArgumentError)
  end
  ...
  it "should cache blocks" do
    ...
    expect(@sharder.connection).to_not receive(:select_one)
    @sharder.block_for_key(1)
  ...
  it "should not cache blocks if asked not to" do
    ...
    expect(@sharder.connection).to receive(:select_one).twice.and_return(block)
